...
export const refreshAccessToken = async (req, res) => {
  ...
  if (!refreshToken || typeof refreshToken !== "string") {
    return res
      .status(400)
      .json({ message: "Formato de token de actualización no válido" });
  }
  try {
...
export const updateUserById = async (req, res) => {
  ...
  try {
    ...
    await pool
      .request()
      .input("nombre", mssql.VarChar, nombre || user.nombre)
      .input("apellido", mssql.VarChar, apellido || user.apellido)
      .input("email", mssql.VarChar, email || user.email)
      .input("telefono", mssql.VarChar, telefono || user.telefono)
      .input("usuario", mssql.VarChar, usuario || user.usuario)
      .input("contraseña", mssql.VarChar, hashedPassword || user.contraseña)
      .input("id", mssql.Int, id)
