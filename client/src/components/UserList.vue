                        ...
                        <template v-slot:top>
                            <v-toolbar flat>
                                ...
                                <v-dialog v-model="dialogDelete" max-width="600px">
                                    <v-card>
                                        <v-card-title class="text-h5">¿Estás seguro que quieres eliminar este
            ...
            if (this.editedItem.usuario.length > 0) {
                try {
                    if (this.editedIndex === -1) {
                        await post('/users', this.editedItem);
                    } else {
                        await put(`/users/${this.editedItem.id}`, this.editedItem);
                    }

                    await this.initialize();
                    this.close();
                } catch (error) {
                    console.log("Error en la función save de UserList:", error);
                    throw error;
                }
            }
        },
